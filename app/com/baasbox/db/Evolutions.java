...
public class Evolutions {
	...
	/***
	 * Executes db evolutions from the given version (excluded) that is supposed
	...
	private static void postEvolutionTasks(ODatabaseRecordTx db) {
		...
		DbHelper.execMultiLineCommands(db, true, "rebuild index *");
		if (BBConfiguration.getInstance().configuration
				.getBoolean(BBConfiguration.getInstance().ORIENT_START_CLUSTER)) {
			// in case of internal clustering support we have to drop some
			// constaints because of
			// http://orientdb.com/docs/last/orientdb.wiki/Graph-Schema.html#constraints
			DbHelper.execMultiLineCommands(db, true, "alter property _BB_Node._links mandatory=false",
	...
	public Evolutions() {
		...
		me.put(ev.getFinalVersion(), ev);
		ev = (IEvolution) new Evolution_a_000_009_005_001();
		me.put(ev.getFinalVersion(), ev);
		ev = (IEvolution) new Evolution_a_000_009_005_010();
		me.put(ev.getFinalVersion(), ev);
		ev = (IEvolution) new Evolution_a_001_000_000_010();
		me.put(ev.getFinalVersion(), ev);
	}
