...
public class Document extends Controller {
	...
	private static Result getDocumentsChunked(String collectionName) throws InvalidAppCodeException {
		...
		QueryParams criteria = (QueryParams) ctx.args.get(IQueryParametersKeys.QUERY_PARAMETERS);
		if (criteria.isPaginationEnabled()) criteria.enablePaginationMore();
		return chunksFromCollection(collectionName, ()->{
			return DbHelper.selectQueryBuilder(collectionName, criteria.justCountTheRecords(), criteria);
		});
	}

	
	private static Result chunksFromCollection(String collectionName,Supplier<String> querySupplier) throws InvalidAppCodeException{
		
		final Context ctx = Http.Context.current.get();
		String select = "";
		try{
			DbHelper.openFromContext(ctx);
			select = querySupplier.get();
	...
	private static Result queryLinksChunked(String collectionName, String id, String linkName,String linkDirection) throws InvalidAppCodeException {
		final Context ctx = Http.Context.current.get();
		QueryParams criteria = (QueryParams) ctx.args.get(IQueryParametersKeys.QUERY_PARAMETERS);
		if (criteria.isPaginationEnabled()) criteria.enablePaginationMore();
		return chunksFromCollection(collectionName, ()->{
			String select = String.format(NodeDao.LINKS_QUERY_FORMAT, linkDirection, linkName, collectionName, id);
			if (StringUtils.isEmpty(criteria.getFields())) {
				criteria.fields("*");
			}
			return DbHelper.selectQueryBuilder(select, criteria.justCountTheRecords(), criteria);
		});
	...
	public static Promise<Result> queryLink(String collectionName, String id, String linkName, String linkDirection) {
		if (!linkDirection.matches(LinkDirection.regexp())) {
			return Promise.pure(badRequest("linkDir param must contain one of the following values: to(default),from or both"));
		}
		return Promise.promise(DbHelper.withDbFromContext(ctx(), () -> {

			if (BBConfiguration.getInstance().isChunkedEnabled() && request().version().equals(HttpConstants.HttpProtocol.HTTP_1_1)) {
				if (BaasBoxLogger.isDebugEnabled()) BaasBoxLogger.debug("Prepare to sending chunked response..");
				return queryLinksChunked(collectionName,id,linkName,LinkDirection.map(linkDirection));
			}
			QueryParams criteria = (QueryParams) ctx().args.get(IQueryParametersKeys.QUERY_PARAMETERS);
